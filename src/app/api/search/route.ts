...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // 1. Search Quran (Surah + Verses)
    if (isAll || type === 'quran') {
      const quranLimit = type === 'quran' ? 15 : 5;
      ...
      const surahRows = db
        ...
        .all(searchTerm, searchTerm, searchTerm, quranLimit) as {
        id: number;
        name_simple: string;
        name_arabic: string;
    ...
    if (isAll || type === 'dua') {
      ...
      const duaRows = db
        ...
        .all(searchTerm, searchTerm, searchTerm, duaLimit) as {
        id: number;
        title: string;
        text_translation: string;
        category: string;
    ...
    // 3. Search Dhikr
    if (isAll || type === 'dhikr') {
      const dhikrLimit = type === 'dhikr' ? 20 : 5;
      const dhikrRows = db
        .prepare(
        ...
        .all(searchTerm, searchTerm, searchTerm, dhikrLimit) as {
        id: number;
        title: string;
        category: string;
        text_translation: string;
